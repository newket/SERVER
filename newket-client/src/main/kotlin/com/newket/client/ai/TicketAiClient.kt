...
- 단, 공지 내용이 n차 티켓오픈 공지인 경우에는 해당 티켓오픈 대상 공연 일정만 추출하세요.
...
반드시 JSON 하나만 출력하세요.
            ...
            val json =
                aiClient.generateContent(prompt, "google/gemma-4-26b-a4b-it")?.replace("`", "")?.replace("json", "")
    ...
    fun getArtists(info: String, artistList: String): List<CreateTicketRequest.Artist> {
        try {
            val prompt =
                ...
                $artistList
                """.trimIndent()
            val json = aiClient.generateContent(prompt, "google/gemma-4-31b-it")?.replace("`", "")?.replace("json", "")
            val objectMapper = ObjectMapper()
            val node = objectMapper.readTree(json)
